...
def get_random_point():
    return int(random.random() * len(etapesARealiser))

def nouvelle_etape(point):
    global tempsTotal
    global tempsVisite
    global etapesARealiser
    global etapesRealisees
    etapesRealisees.append(point)
    if len(etapesARealiser) > 0:
        del etapesARealiser[point]
...
def envoyer_fourmis(teleportations, distances, temps):
...
while envoyer_fourmis(nbrTeleportation, distanceParcourue, tempsTotal):

    etapesARealiser = list(places)
    etapesRealisees = []
    tempsTotal = 0
    nbrTeleportation = 0
    distanceParcourue = 0
    ...
    stop = False
    while True:
        if len(etapesARealiser) is not 0:
